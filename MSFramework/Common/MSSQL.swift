...
public final class MSSQL
{
    ...
    {
        guard !appendedSQL.contains(where: {
            return $0.formattedStatement == sqlStatement.formattedStatement
        }) else { return }
    ...
    private func check(value: inout String, equivalence: MSSQLEquivalence = .lessThan) throws
    {
    ...
    public func join(_ join: MSSQLJoin, joins: [Join]) throws -> MSSQL
    {
        ...
        for join in joins
        {
            ...
            try check(attribute: join.tableTwoAttribute)
        }
