...
@available(*, unavailable, renamed: "MSFrameworkDataSource")
public typealias MSDatabaseDataSource = MSFrameworkDataSource

/**
 Used by MSFamework to convert a MySQL table and its attributes to their corresponding CoreData entities and attributes
 ...
 * An array of `MySQLToCoreData` objects that allows MSFramework to auto-save downloaded data
 */
public protocol MSFrameworkDataSource
{
    ...
    /**
     The relative path to a file in the URL that takes a `POST` object containing `databaseUserPass`, `websiteUserName`, and an SQL statement and processes the SQL statement returning the new user's ID if the SQLStatement is successfully ran or **"Failure"** if it fails
    ...
    /**
     ...
     MSFramework attempts to automatically find the CoreData information.  If it cannot, MSFramework will call upon this variable to retrieve the right information
     */
