...
@implementation NSData (MSEncryption)
...
- (NSData *) AES256EncryptedDataUsingKey:(id)key error:(NSError **)error
{
    ...
    NSData * result = [self dataEncryptedUsingAlgorithm: kCCAlgorithmAES128 key: key options: kCCOptionPKCS7Padding error: &status];
...
{
    CCCryptorStatus status = kCCSuccess;
    NSData * result = [self decryptedDataUsingAlgorithm: kCCAlgorithmAES128 key: key options: kCCOptionPKCS7Padding error: &status];
...
static void FixKeyLengths(CCAlgorithm algorithm, NSMutableData *keyData, NSMutableData *ivData)
{
    ...
    switch (algorithm)
    {
        ...
        {
            if (keyLength <= 16) [keyData setLength: 16];
            
            else if (keyLength <= 24) [keyData setLength: 24];
        ...
        {
            if (keyLength <= 5) [keyData setLength: 5];
            
            else if (keyLength > 16) [keyData setLength: 16];
